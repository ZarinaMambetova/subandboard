...
// Копируем фото
task('copy:images', () => {
  return src(`${SRC_PATH}/images/pic/**/*.*`).pipe(dest(`${DIST_PATH}/images/pic`)).pipe(reload({ stream: true }));
});
...
task('build', series('clean', parallel('copy:html', 'copy:images', 'copy:fonts', 'styles', 'scripts', 'icons')));
